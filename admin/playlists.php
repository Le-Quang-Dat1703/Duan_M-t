...
if (isset($_POST['delete_playlist'])) {
   ...
   if ($verify_playlist->rowCount() > 0) {
      ...
      if ($prev_thumb != '' && file_exists('../uploaded_files/'.$prev_thumb)) {
...
<section class="playlists">
   <h1 class="heading">Allll Playlist</h1>
   <div class="box-container">
      <!-- <div class="box" style="text-align:center;">
         <h3 class="title" style="padding-bottom: .7rem;">created new playlist</h3>
         ...
         <?php
            ...
            $select_contents = $conn->prepare("SELECT * FROM `content` WHERE playlist_id = ?");
            $select_contents->execute([$playlist_id]);
            $total_contents = $select_contents->rowCount();
         ?>
         <div class="box">
            <div class="flex">
               <div><i class="fas fa-circle-dot" style="color:<?php echo $fetch_playlist['status'] == 'active' ? 'limegreen' : 'red'; ?>"></i><span style="color:<?php echo $fetch_playlist['status'] == 'active' ? 'limegreen' : 'red'; ?>"><?= $fetch_playlist['status'] ?></span></div>
